...
function App() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log('Feedback Summary Result:', summary);
  ...
  const getFeedbackSummary = async (transcript) => {
    try {
      console.log('Fetching feedback for transcript:', transcript);
      ...
      if (res.ok && data?.choices?.[0]?.message?.content) {
        ...
        console.error('Feedback API Error:', data.error || 'Unknown error');
        setMessage(`Failed to fetch feedback: ${data.error || 'Unknown error'}`);
